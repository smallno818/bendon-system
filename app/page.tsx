...
// ★ 修改 Order 定義，加入 quantity
type Order = { id: number; item_name: string; price: number; customer_name: string; quantity: number; };
// ★ 修改 SummaryItem 定義，記錄每筆訂單的數量以便顯示 (x10)
...
export default function Home() {
  ...
  const calculateSummary = (ordersData: Order[]) => {
    ...
    ordersData.forEach(order => {
      ...
      // 紀錄誰點了幾杯
      stats[order.item_name].orderDetails.push({ 
        id: order.id, 
    ...
    <div className="min-h-screen bg-gray-50 pb-20 relative">
      {!currentStore ? (
        ...
        <>
          ...
          <button onClick={scrollToTop} className={`fixed bottom-28 right-8 z-40 bg-gray-700/80 text-white p-3 rounded-full shadow-lg backdrop-blur-sm hover:bg-gray-900 transition-all duration-300 print:hidden ${showScrollTop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`} title="回到頂部">
